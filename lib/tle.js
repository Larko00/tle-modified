...
class TLE {
  ...
  parse( value ) {
    ...
    this.tle2 = line
    this.tleArr = [this.tle1, this.tle2]
    try {
      this.year = getEpochYear(this.tleArr)
      this.day = getEpochDay(this.tleArr)
      this.timestamp = getEpochTimestamp(this.tleArr)
      this.orbitmodel = getOrbitModel(this.tleArr)
      this.designator = getIntDesignatorYear(this.tleArr)
      this.info = getSatelliteInfo(this.tleArr)
    } catch(err) {
      console.log("error")
    }
